        ...
        if not viewed_categories_list:
            ...
            try:
                ...
                if X_predict.nnz == 0:
                    ...
                    st.subheader("Predicted Category Probabilities:")
                    class_labels = model.classes_
                    prob_df = pd.DataFrame({'Category': class_labels, 'Probability': probabilities})
                    prob_df = prob_df.sort_values('Probability', ascending=False).head(3)  # Top 3

                    # Create bar chart
                    plt.figure(figsize=(10, 6))
                    sns.barplot(x='Probability', y='Category', data=prob_df, palette='viridis')
