        ...
        if not viewed_categories_list:
            ...
            try:
                ...
                if X_predict.nnz == 0:
                    ...
                    # Chart.js visualization with alternative CDN and debugging
                    chart_data = {
                        "labels": list(top_categories),
                        "probs": list(top_probs),
                        "selected": ', '.join(viewed_categories_list)
                    }
                    st.write("Debug: Chart data:", chart_data)  # Debugging output
                    chart_html = f'<html>\
<head>\
        ...
        try {{ \
            ...
            console.error("Chart.js error: " + e.message);\
                    ...
                    try:
                        ...
                        st.warning(f"Failed to render chart: {str(e)}. Please check the browser console for JavaScript errors.")
            except Exception as e:
